...
void runYOLO(vart::Runner* runner,float conf,float rmax, float bmax,Mat& img) {
  ...
  for (int io = 0; io < 3; io++) {
    ...
    output_scale.push_back(get_output_scale(
        runner->get_output_tensors()[shapes.output_mapping[io]]));
  }
...
int main(int argc, char *argv[]) {
    ...
    auto now_b_while = std::chrono::system_clock::now();

    // Convert the current time to time since epoch
    auto duration_b_while  = now_b_while .time_since_epoch();

    // Convert duration to milliseconds
    auto milliseconds_b_while 
    ...
    int t_check = 1;
        ...
        while (!signal_caught && camera.is_running()) {
...

            {
             ...
             auto now_in_while = std::chrono::system_clock::now();

    // Convert the current time to time since epoch
    auto duration_in_while  = now_in_while .time_since_epoch();

    // Convert duration to milliseconds
    auto milliseconds_in_while
        = std::chrono::duration_cast<std::chrono::milliseconds>(
            duration_in_while )
            .count();

    auto elapsedTime = milliseconds_in_while -milliseconds_b_while ; 
        if (elapsedTime >= (interval*t_check)) {
